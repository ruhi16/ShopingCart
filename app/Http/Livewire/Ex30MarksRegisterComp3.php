...
class Ex30MarksRegisterComp3 extends Component
{
    // Filters
    ...
    public function generatePdf($myclassId = null)
    {
        ...
        // Calculate and sort students by overall marks (ranking)
        $studentsWithRank = [];
        foreach ($students as $student) {
            ...
            $studentsWithRank[] = ['student' => $student, 'overall_obt' => $overallObt];
        }
        
        usort($studentsWithRank, function($a, $b) {
            return $b['overall_obt'] - $a['overall_obt'];
        });
        ...
        if ($totalStudents > 0 && $examColCount > 0) {
            ...
            // Student rows - write one student at a time (sorted by overall marks)
            foreach ($studentsWithRank as $rankData) {
                $student = $rankData['student'];
                $studentcrId = $student->id;
                $studentdbId = $student->studentdb_id;
    ...
    private function loadRegisterData()
    {
        ...
        // Sort students by overall marks (ranking)
        $examDetailsLocal = $this->examDetails;
        $this->students = $this->students->map(function ($student) use ($studentSubjects, $allMarks, $examSettingsByDetail, $examDetailsLocal) {
            $studentcrId = $student->id;
            $studentdbId = $student->studentdb_id;
            $subjectsOpted = $studentSubjects->get($studentdbId, collect());
            ...
            foreach ($subjectsOpted as $so) {
                ...
                foreach ($examDetailsLocal as $ed) {
                    $me = $allMarks->get($studentcrId, collect())
                        ->where('exam_detail_id', $ed->id)
                        ->where('subject_id', $sid)
                        ->first();
                    ...
                    if ($me && $me->marks_obtained !== null && !($me->is_absent ?? false)) {
                        $subjObt += $me->marks_obtained;
                        $subjFull += $setting ? ($setting->full_mark ?? 0) : 0;
                    }
                }
                
                $subjectTotals[$sid] = ['obt' => $subjObt, 'full' => $subjFull];
            }
            
            // Find additional subject (lowest marks)
            if (!empty($subjectTotals)) {
                $minVal = min(array_column($subjectTotals, 'obt'));
                foreach ($subjectTotals as $sid => $val) {
                    if ($val['obt'] == $minVal) {
            ...
            // Calculate overall excluding additional subject
            foreach ($subjectTotals as $sid => $val) {
                if (isset($additionalSubjectId) && $sid == $additionalSubjectId) continue;
                $overallObt += $val['obt'];
                $overallFull += $val['full'];
            }
            
            $student->overall_obt = $overallObt;
            $student->overall_full = $overallFull;
            return $student;
        })->sortByDesc('overall_obt')->values();
    }
