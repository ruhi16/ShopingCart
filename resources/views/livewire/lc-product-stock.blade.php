...
<body class="bg-gray-50 p-4">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Calculate conversion totals on hover
            rows.forEach(row => {
                row.addEventListener('mouseenter', function() {
                    if (this.classList.contains('unit-conversion')) {
                        const conversionRatio = this.querySelector('td:nth-child(9)').textContent.trim();
                        const availableUnits = this.querySelector('td:nth-child(10)').textContent.trim();
                        const profitPerUnit = this.querySelector('td:nth-child(13)').textContent.trim();
                        
                        // Show tooltip with conversion details
                        this.title = `Conversion: ${conversionRatio} | Available: ${availableUnits} units | Profit per unit: ${profitPerUnit}`;
                    }
                });
            });
            ...
            // Calculate and display real-time profit calculations
            function updateProfitCalculations() {
                const totalInvestment = Array.from(document.querySelectorAll('td:nth-child(7)')).reduce((sum, cell) => {
                    const value = parseFloat(cell.textContent.replace(/[₹,]/g, '')) || 0;
                    return sum + value;
                }, 0);

                const totalRevenue = Array.from(rows).reduce((sum, row) => {
                    const availableUnits = parseFloat(row.querySelector('td:nth-child(10)').textContent) || 0;
                    const sellingPrice = parseFloat(row.querySelector('td:nth-child(12)').textContent.replace(/[₹,]/g, '')) || 0;
                    return sum + (availableUnits * sellingPrice);
                }, 0);

                // Update summary cards if needed
                console.log(`Total Investment: ₹${totalInvestment.toLocaleString()}`);
                console.log(`Expected Revenue: ₹${totalRevenue.toLocaleString()}`);
            }
